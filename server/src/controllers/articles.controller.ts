...
import { IArticles, IGenerics } from '~/interfaces';
...
export class ArticlesController {
  private defaultOptions: request.OptionsWithUri = {
    ...
    json: true,
    baseUrl: this.configService.get('NYT_HOST_URL'),
  };
  ...
  async getArticles(@Params() params) {
    ...
    let results: IArticles | undefined;
    const cache = (await this.cacheService.get('allSections')) as IArticles;
    if (cache) {
      results = cache;
    } else {
